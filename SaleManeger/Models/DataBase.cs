			...
			using (SQLiteConnection connection = new SQLiteConnection(connectionString))
			{
				...
				using (SQLiteCommand command = new SQLiteCommand("SELECT ClientID FROM Clients WHERE ClientID == @ID", connection))
				{
					...
					using (var reader = command.ExecuteReader())
					{
						...
						{
							//Update client
							using (SQLiteCommand updateClientCommand = new SQLiteCommand("DELETE FROM ClientOrder WHERE ClientID = @ClientID AND SaleID = @SaleID", connection))
							{
								updateClientCommand.Parameters.AddWithValue("@ClientID", client.ID);
								updateClientCommand.Parameters.AddWithValue("@SaleID", saleName);
